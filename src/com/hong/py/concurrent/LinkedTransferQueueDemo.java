...
public class LinkedTransferQueueDemo {
    ...
    public static void main(String[] args){
        ...
        Thread product1=new Thread(()->{
            String take = null;
            try {
                take = queue.take(); //会阻塞在这里直到。queue.transfer("第二次");
                System.out.println(take);
            } catch (InterruptedException e) {
                e.printStackTrace();
            }
        ...
        Thread consumer=new Thread(()->{
            ...
            try {
                //若当前存在一个正在等待获取的消费者线程，即立刻将e移交之；
                // 否则将元素e插入到队列尾部，
                // 并且当前线程进入阻塞状态，直到有消费者线程取走该元素
                queue.transfer("第二次");

            } catch (InterruptedException e) {
                e.printStackTrace();
            }
            System.out.println("生产者1结束");
        });
        consumer.start();
